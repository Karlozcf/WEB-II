...
// Adicione esta linha para incluir o arquivo de conexão
...
if (isset($_GET["cafe"]) && isset($_GET["qtd"])) {
    $idCafe = intval($_GET["cafe"]);
    $qtd = intval($_GET["qtd"]);

    try {
        $stmt = $pdo->prepare("SELECT nome, valor FROM produtos WHERE id = ?");
        $stmt->execute([$idCafe]);
        $produto = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($produto) {
            $nomeCafe = $produto["nome"];
            $valorCafe = floatval($produto["valor"]);
...
// Repita a mesma lógica para as "comidas"

if (isset($_GET["comida"]) && isset($_GET["qtde"])) {
    $idComida = intval($_GET["comida"]);
    $qtde = intval($_GET["qtde"]);

    try {
        $stmt = $pdo->prepare("SELECT nome, valor FROM produtos WHERE id = ?");
        $stmt->execute([$idComida]);
        $produto = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($produto) {
            $nomeComida = $produto["nome"];
            $valorComida = floatval($produto["valor"]);
            $subTotal = ($valorComida * $qtde);
